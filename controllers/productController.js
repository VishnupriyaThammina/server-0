...
const AddProduct = async(req,res)=>{
    try{
        ...
        if(existsProduct){
           ...
           return res.status(409).json({message:"product all ready exists in database"})
       ...
       return res.status(200).json({message:'successfully saved data to database',product:product})
...
const AllProducts = async(req,res)=>{
    try{
        const products = await Product.find({}, { name: 1, pid: 1, description: 1,images:1,thumbnail:1,_id:0 });
        res.status(200).json({products:products,message:"Fetched all products information"})
    }catch(err){
        console.log('internal server error incurred while adding a product')
...
if(!existsProduct){
   ...
   return res.status(409).json({message:"product is not present in database"})
    // 409 represents conflict in current request body product id 
}
...
const updatedProduct = await Product.findOneAndUpdate({pid:pid},// filter object is expected 
    updateFields, // feilds to be updated
    {new:true} // return the updated document
)

res.status(200).json({updatedProduct:updatedProduct,message:"Update succesfull"})
    }catch(err){
